...
func TestRepository_UpsertAndSearchScoped(t *testing.T) {
	...
	require.NoError(t, repo.Upsert(requestContext, patient))
	firstID := patient.ID
	...
	require.NoError(t, repo.Upsert(requestContext, otherHospitalPatient))

	resultsA, operationError := repo.Search(requestContext, "hospital-a", domain.SearchCriteria{NationalID: strPtr("integ-nid-001")})
	require.NoError(t, operationError)
	require.Len(t, resultsA, 1)
	require.Equal(t, "hospital-a", resultsA[0].Hospital)

	resultsB, operationError := repo.Search(requestContext, "hospital-b", domain.SearchCriteria{NationalID: strPtr("integ-nid-001")})
	require.NoError(t, operationError)
